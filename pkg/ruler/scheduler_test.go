...
func TestSchedulerRulesOverlap(t *testing.T) {
	...
	cfg := userRules{generation: 1, ruleList: ruleSets}
	s.rulesList[userID] = cfg
	w1 := workItem{userID: userID, ruleGroupName: groupName, scheduled: next, generation: cfg.generation}
	s.workItemDone(w1)
	item := s.q.Dequeue().(workItem)
	assert.Equal(t, w1.generation, item.generation)

	w0 := workItem{userID: userID, ruleGroupName: groupName, scheduled: next, generation: cfg.generation - 1}
	s.workItemDone(w1)
	s.workItemDone(w0)
	item = s.q.Dequeue().(workItem)
	assert.Equal(t, w1.generation, item.generation)
